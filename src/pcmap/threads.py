...
def isTransformAsVector(parameters):
    ...
    """
    return (
        "eulers" in parameters
        and "translations" in parameters
        and "offsetRec" in parameters
        and "offsetLig" in parameters
    )
...
def isExplicitAsVector(parameters):
    ...
    """
    return ("pdbAtomListREC" in parameters and "pdbAtomListLIG" in parameters) or (
...
def run(threadNum=8, **kwargs):
    ...
    if isTransformAsFile(kwargs):
        ...
        threadNum, threadArgs, _threadKwargs = lzGenerateArgsFromfile(
            kwargs["transformation"], kwargs["pdbA"], kwargs["pdbB"], threadNum
        )
    ...
    # Create Thread parameters for list of PDB files path extracted from file
        ...
        wThread = lcThread
        threadNum, threadArgs = lcGenerateArgsFromFile(kwargs["structList"], threadNum)
    ...
    # as atom vectors
        ...
        threadNum, threadArgs = lcGenerateArgsFromVector(*args, threadNum=threadNum)
    else:
    ...
    threadPool = [
        threading.Thread(
            args=tuple([threadArgs[i], i, output]), kwargs=threadKwargs, target=wThread
        )
    ...
    # print(
    #    "Output type:", "deserialize" if threadKwargs["deserialize"] else "serialized"
    # )
